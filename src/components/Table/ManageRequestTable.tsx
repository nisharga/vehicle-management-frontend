...
import { Input, PaginationProps, Popconfirm, Select, message, Modal, Button, Form, Spin } from "antd";
...
import { ManageRequestTableFields, ManageRoleListTableFields } from "./StaticTableData";
import { useGetManageRequestsQuery, useUpdateRequestMutation } from "@/redux/api/manageRequest";
...
const ManageRequestTable = () => {
    ...
    const {
        ...
        //   console.log('afterSubmit', comment,'Id',commentId);
          ...
          await updateRequest({ id: commentId, data: { approve_status: 'REJECT',comment:comment } });
          return 'ok';
        },
        form,
      });
    ...
    const handleChange = async (value: string) => {
        ...
        try {
            if (tempData[1] === 'REJECT') {
                ...
                const res = await updateRequest({ id: tempData[0], data: { approve_status: tempData[1] } });
                console.log(res)
                        ...
                        <tbody className="dark:text-[#E8E8E8]">
                            {((manageRequests as any)?.data ?? [])?.map(
                                (request: IProps, index: number) => (
                                    <tr
                                        key={request?.id}
                                        className={`${index % 2 === 0 ? "" : "bg-gray-50 dark:bg-[#145374]"
                                            }  `}
                                    >
                                        ...
                                        <td className="px-2 py-3 text-sm leading-5">
                                            {request?.comment === null ? 'No comment' : request?.comment}
                                        </td>
